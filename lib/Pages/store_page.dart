...
class _StorePageState extends State<StorePage> with TickerProviderStateMixin {
  ...
  Future<void> _loadInventory() async {
    ...
    if (result.success && result.data != null) {
      ...
      _showSnackBar('Error cargando inventario: ${result.message}', Colors.orange);
    }
  }
  ...
  List<Materials> get _filteredMaterials {
    ...
    return _materials.where((material) {
      return material.cod.toLowerCase().contains(_searchQuery.toLowerCase()) ||
             (material.descripcion?.toLowerCase().contains(_searchQuery.toLowerCase()) ?? false);
    }).toList();
  }
  ...
  Future<void> _deleteMaterial(Materials material) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('¿Estás seguro de que quieres eliminar el material ${material.cod}?\nEsto también lo eliminará del inventario si existe.'),
        actions: [
          TextButton(
  ...
  Future<void> _addToInventory(Materials material) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('¿Quieres agregar ${material.cod} al inventario como disponible?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(false),
            child: Text('Cancelar'),
                  ...
                  : RefreshIndicator(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final isInInventory = _inventory.any((item) => item.cod == material.cod);
                          ...
                          return Card(
                            ...
                            child: ListTile(
                              ...
                              leading: Container(
                                ...
                                child: Icon(
                                  isInInventory ? Icons.inventory : Icons.inventory_outlined,
                                  color: Colors.white,
                                ),
                              ),
                              title: Row(
                                  ...
                                  Row(
                                    children: [
                                      ...
                                      if (material.fechaIni != null) ...[
                                        ...
                                        Text('${material.fechaIni!.day}/${material.fechaIni!.month}/${material.fechaIni!.year}'),
                                      ],
                                    ],
                                  ),
                                ],
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: EstadoMaterial.values.map((estado) {
            return ListTile(
              title: Text(_getEstadoText(estado)),
              leading: Icon(Icons.circle, color: _getColorForEstado(estado)),
              selected: estado == item.estado,
              onTap: () async {
                Navigator.of(context).pop();
                if (estado != item.estado) {
                  final result = await _materialService.updateInventoryStatus(item.cod, estado);

                  print("resultttt: $result");
                  
                  if (result.success) {
                    _showSnackBar('Estado actualizado exitosamente', Colors.green);
                    _loadInventory();
                  } else {
                    _showSnackBar(result.message, Colors.red);
                  }
  ...
  Future<void> _removeFromInventory(MaterialEnInventario item) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('¿Estás seguro de que quieres quitar ${item.cod} del inventario?\nEl material base seguirá existiendo.'),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(false),
            child: Text('Cancelar'),
